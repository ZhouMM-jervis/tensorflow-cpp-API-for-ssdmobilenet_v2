...
cv::String DEFAULT_IMAGE_PATH = "PATH/TO/IMAGE/FILE/*.png";
//cv::String OUTPUT_IMAGE_PATH = "/home/dsai/thirdparty/tensorflow_cc/example/result/";

#pragma mark - Constants: Tensorflow
std::string TF_PB_PATH = "PATH/TO/CHECKPOINT/FILE/frozen_inference_graph.pb";
std::string TF_LABELLIST_PATH = "PATH/TO/PROJECT/kitti_label_map.txt";
float const TF_PREDICTION_THRESSHOLD = 0.005;
...
int main(int argc, const char * argv[]) {
    ...
    cv::glob(imagePath, fn, false);
    std::size_t count = fn.size();
    for (int i = 0; i < count; i++){
       tm.reset();
       tm.start();
       cv::Mat image = cv::imread(fn[i],CV_LOAD_IMAGE_COLOR);
       ...
       //OUTPUT_IMAGE_PATH = OUTPUT_IMAGE_PATH +（str)i + ".png"
       //cv::imwrite(OUTPUT_IMAGE_PATH, image, compressionParams);
       cv::namedWindow("Display window", cv::WINDOW_AUTOSIZE);
       cv::imshow("Display window", image);
       tm.stop();  
       std::cout << "Infer time, ms: " << tm.getTimeMilli()  << std::endl;
       cv::waitKey(0);
    }  
    return EXIT_SUCCESS;
}
